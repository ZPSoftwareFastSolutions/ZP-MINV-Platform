/**
 * M-INV V2 · DiagnosticoInstalacion.ts — prueba de humo en el entorno real (ejecútelo una vez tras instalar)
 ...
 * Microsoft 365, que su usuario esté autorizado y tenga fila de captura, que la contraseña de los scripts coincida
 * con la del libro (pausa y reanuda la protección de cada hoja) y que existan todas las tablas.
 * El informe queda en la salida del editor de código y en la hoja técnica 92_SESION (columna A).
 */
function main(workbook: ExcelScript.Workbook): string {
  const informe: string[] = ["M-INV V2 · Diagnóstico de instalación · " + new Date().toLocaleString()];
  ...
  for (const h of ["00_PORTADA_BODEGA", "00_PORTADA_VENTAS", "01_CONFIG", "02_USUARIOS", "10A_ENTRADAS",
    "10B_SALIDAS", "15_STOCK", "16_ALERTAS", HOJA_SESION]) {
  ...
  for (const t of ["tblUsuarios", "tblProductos", "tblUnidades", "tblTiposMov", "tblEstados", "tblCapturaEntradas",
    "tblCapturaSalidas", "tblEntradas", "tblSalidas", "tblStock", "tblAlertas"]) {
    if (workbook.getTable(t)) {
      ok("Tabla " + t);
    } else {
      mal("Falta la tabla " + t);
    }
  }
  ...
  if (yo) {
    ...
    if (!u) {
      mal("Su cuenta no está en 02_USUARIOS: agréguela para poder registrar");
    } else {
      if (u.activo) {
  ...
  for (const h of ["10A_ENTRADAS", "10B_SALIDAS", "15_STOCK", "16_ALERTAS", "01_CONFIG"]) {
    try {
  ...
  informe.push(fallas === 0 ? "RESULTADO: instalación correcta" : "RESULTADO: " + fallas + " problema(s) por resolver");
