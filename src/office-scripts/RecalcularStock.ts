...
function main(workbook: ExcelScript.Workbook): string {
  ...
  let yo: Identidad = { correo: "(no identificado)", nombre: "" };
  try {
    yo = identificarUsuario(workbook);
  } catch (error) {
    console.log("Recalcular: sin identidad (" + mensajeDe(error as Error) + ")");
  }
  ...
  for (const nombre of ["tblEntradas", "tblSalidas"]) {
    ...
    for (let i = 0; i < skus.length; i++) {
      ...
      procesados++;
      const neta = numero(netas[i]) || 0;
      if (neta >= 0) {
        entradas.set(sku, (entradas.get(sku) || 0) + neta);
      } else {
        salidas.set(sku, (salidas.get(sku) || 0) - neta);
      }
      const f = numero(fechas[i]);
  ...
  for (const p of productos) {
    ...
    const nivel: Celda = p.maximo > 0 ? r6(Math.max(0, stock) / p.maximo) : "";
...
function accionesPorEstado(workbook: ExcelScript.Workbook): Map<string, string> {
  const t = leerTabla(workbook, "tblEstados");
  const cE = columna(t, "Estado"), cA = columna(t, "Acción");
  const m = new Map<string, string>();
  for (const f of t.filas) {
    m.set(texto(f[cE]), texto(f[cA]));
  }
  return m;
}
...
function rellenar(filas: Celda[][], capacidad: number, ancho: number, hojaNombre: string): Celda[][] {
  if (filas.length > capacidad) {
    throw new Error(hojaNombre + " admite " + capacidad + " filas y se necesitan " + filas.length + ".");
  }
  const bloque: Celda[][] = filas.map((f: Celda[]) => f.slice(0, ancho));
  while (bloque.length < capacidad) {
    const vacia: Celda[] = [];
