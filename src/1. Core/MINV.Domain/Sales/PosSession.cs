...
public sealed class PosSession : Entity, IConcurrencyAware, IAggregateRoot
{
    private PosSession()
    {
    }

    private PosSession(Guid tenantId, Guid posRegisterId, Guid openedByUserId, DateTimeOffset openedAt, decimal openingCash)
        : base(tenantId)
    {
    ...
    public static PosSession Open(Guid tenantId, Guid posRegisterId, Guid userId, decimal openingCash, DateTimeOffset now) =>
        new(tenantId, posRegisterId, userId, now, openingCash);
    ...
    {
        EnsureOpen();
        return new CashMovement(TenantId, Id, direction, amount, reason, now, userId);
    }
