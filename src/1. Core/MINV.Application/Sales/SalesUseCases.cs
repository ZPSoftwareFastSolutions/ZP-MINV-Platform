...
public sealed class CheckoutHandler(IMinvDbContext db, ICurrentUser user, IClock clock) : IRequestHandler<CheckoutCommand, CheckoutResult>
{
    public async Task<CheckoutResult> Handle(CheckoutCommand request, CancellationToken ct)
    {
        ...
        for (var attempt = 1; ; attempt++)
        {
            try
            {
                return await CheckoutAsync(request, userId, ct);
            }
    ...
    private async Task<CheckoutResult> CheckoutAsync(CheckoutCommand r, Guid userId, CancellationToken ct)
    {
        ...
        var customerCode = r.CustomerCode.Trim().ToUpperInvariant();
        ...
        var methodCode = r.PaymentMethodCode.Trim().ToUpperInvariant();
        ...
        Guard.That(!method.RequiresReference || !string.IsNullOrWhiteSpace(r.PaymentReference), "payment.reference",
        ...
        var orderNumber = await Documents.NextNumberAsync(db.Set<SalesOrder>(), o => o.Number, "PV", ct);
        var invoiceNumber = await Documents.NextNumberAsync(db.Set<Invoice>(), i => i.Number, "F", ct);
        // Arco exclusivo: la venta de caja se asocia a la sesión (el almacén sale de la caja), nunca a ambos
        var order = new SalesOrder(customer.TenantId, orderNumber, customer.Id, session.Id, null, priceList.Id, today);
        ...
        foreach (var (line, item) in items)
        {
            ...
            var context = new MovementContext(user.UserId!.Value, today, now, invoiceNumber, $"Venta {orderNumber} · {customer.Name}");
        ...
        var invoice = new Invoice(customer.TenantId, invoiceNumber, order.Id, null);
        var tax = 0m;
        ...
        var change = 0m;
        if (method.OpensCashDrawer && r.CashReceived is { } received)
        {
        ...
        db.Set<Payment>().Add(new Payment(customer.TenantId, invoice.Id, method.Id, total, r.PaymentReference, now, session.Id, userId));
        await JournalPoster.PostAsync(db, customer.TenantId, userId, today, $"Venta {invoiceNumber} · {customer.Name} · {method.Name}",
        [
        ...
        return new CheckoutResult(invoiceNumber, orderNumber, now, total, JournalPoster.Money(tax), change, customer.Name, method.Name,
...
public sealed class VoidSaleHandler(IMinvDbContext db, ICurrentUser user, IClock clock) : IRequestHandler<VoidSaleCommand, string>
{
    public async Task<string> Handle(VoidSaleCommand request, CancellationToken ct)
    {
        ...
        await JournalPoster.PostAsync(db, invoice.TenantId, userId, today, $"Anulación de la venta {invoice.Number}: {request.Reason.Trim()}",
        [
